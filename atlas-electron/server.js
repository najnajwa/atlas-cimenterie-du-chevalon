...
const PREVIEWS  = path.join(UPLOADS, '_previews'); // high-quality display images
...
[UPLOADS, THUMBS, PREVIEWS, ASSETS].forEach(d => { if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true }); });
...
// High-quality preview for display (1920px max, 92% quality) — works for all formats including NEF
async function makePreview(src, dest) {
  try {
    if (Buffer.isBuffer(src)) await sharp(src).resize(1920,1920,{fit:'inside'}).jpeg({quality:92}).toFile(dest);
    else await sharp(src).resize(1920,1920,{fit:'inside'}).jpeg({quality:92}).toFile(dest);
  } catch {}
}
...
app.post('/api/upload', upload.array('photos',200), async (req,res) => {
  ...
  for (let i=0;i<files.length;i++) {
    ...
    const photo={
      ...
      preview:jpegBuf?`/uploads/_previews/${previewName}`:null,
...
app.post('/api/photos/:id/reclassify', async (req,res) => {
  ...
  try {
    ...
    if(!data.photos[idx].preview) data.photos[idx].preview=`/uploads/_previews/${prevName}`;
